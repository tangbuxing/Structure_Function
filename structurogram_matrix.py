...
def structurogram_matrix(grd, q = 2, R = 5, dx = 1, dy = 1, zero_out = False):
    dat = grd.copy()
    out = {}
    '''
    if (zero.out) 
    ...
    d <- sort(d)
    '''
    d = np.sqrt(np.abs(dx * ind[:, 0])**2 + np.abs(dy * ind[:, 1])**2)
    good = (d > 0) & (d <= R)
    ind = ind[good, :]
    d = d[good]
    ...
    for k in range(nbin):
        ...
        BigDiff = dat_1 - dat_2
        holdVG[k] = np.mean(0.5 * BigDiff**q)
    '''
    ...
    '''
    #dimnames(vgram) <- NULL
    out = {"vgram":vgram, "d":dcollapsed, "ind":ind, "d.full":d, "q":q, 
           "vgram.full":holdVG, "N":holdN, "dx":dx, "dy":dy, "class":"vgram.matrix"}    
        
    return out
    
if __name__ == "__main__":
    dat = pd.read_csv("F:\\Work\\MODE\\tra_test\\FeatureFinder\\pert000.csv")
    look_SGM = structurogram_matrix(grd = dat.values)    #读入的是dataframe数据，有行列名称    
